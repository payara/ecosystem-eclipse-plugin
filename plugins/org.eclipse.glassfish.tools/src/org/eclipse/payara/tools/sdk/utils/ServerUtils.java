...
public class ServerUtils {
	...
	public static class GlassFishFilter implements FileFilter {
		 ...
		 * @return Returns <code>true</code> when given <code>File</code> should be
		 *         included or <code>false</code> otherwise.
		 */
		@Override
		public boolean accept(final File path) {
			if (path.isDirectory() && path.canRead()) {
				File commonUtilJar = getCommonUtilJarInModules(path.getAbsolutePath());
	...
	private static class VersionFilter implements FileFilter {
		 ...
		 * 
		 * @param namePattern
		 *            GlassFish servers file name pattern.
		 */
	...
	/**
	 * GlassFish Java VM system environment <code>AS_JAVA</code> variable name. This
	 * value should be equal to system environment <code>JAVA_HOME</code> value.
	 */
	 ...
	 * <code>glassfish</code> subdirectory under server installation root which we
	...
	public static final String GFV3_JAR_MATCHER = "glassfish" + GFV3_VERSION_MATCHER;

	/**
	 * Manifest attribute Bundle-Version containing GlassFish version
	 * <code>String</code>.
	 */
	...
	/**
	 * Regex pattern to retrieve version string like 3.1.2.2 from full version
	 * string.
	 */
	...
	/**
	 * GlassFish server service response while server is not yet ready. Copy-pasted
	 * from <code>com.sun.enterprise.v3.admin.AdminAdapter</code>.
	 */
	...
	/**
	 * REGEX expression used to split resources returned in <code>Manifest</code>
	 * object from HTTP response.
	 */
	...
	/**
	 * REGEX expression used to extract additional containers from containers list
	 * of <code>Manifest</code> attribute.
	 */
	 ...
	 * <code>Manifest</code> attribute.
	 */
	private static final Pattern MANIFEST_COMPONENT_FULL_PATTERN = Pattern.compile(MANIFEST_COMPONENT_FULL_REGEX,
			Pattern.CASE_INSENSITIVE);

	/**
	 * REGEX pattern used to extract additional containers from containers list of
	 * <code>Manifest</code> attribute.
	 */
	private static final Pattern MANIFEST_COMPONENT_COMP_PATTERN = Pattern.compile(MANIFEST_COMPONENT_COMP_REGEX,
			Pattern.CASE_INSENSITIVE);
	...
	/**
	 * Search for <code>.jar</code> file matching given pattern in given directory
	 * tree.
	 * <p/>
	 * 
	 * @param serverHome
	 ...
	 *            File name pattern.
	 * @param dir
	 *            Directory tree root to be searched for <code>.jar<code> file.
	 * @return <code>File</code> object containing full <code>.jar<code> file
	 *         path or <code>null</code> if no file was found.
	 */
	public static File getJarName(final String serverHome, final String pattern, final String dir) {
	 ...
	 * @param dir
	 *            Directory tree root to be searched for pattern.
	 * @return <code>File</code> object that matches given <code>pattern</code> or
	...
	/**
	 * Append next path element to existing path in <code>StringBuilder</code>. Path
	 * separator is added only when there is no one at the end of existing path.
	 * <p/>
	 * 
	 * @param rootPath
	 *            Already existing path in <code>StringBuilder</code>.
	 * @param next
	 *            Path to be appended at the end of already existing path.
	 */
	public static void addPathElement(final StringBuilder rootPath, final String next) {
		int rootPathLength = rootPath.length();
		if (rootPathLength > 0
				&& rootPath.lastIndexOf(File.separator) + OsUtils.FILE_SEPARATOR_LENGTH != rootPathLength) {
	 ...
	 * @return Path to the <code>common-util.jar</code> file in GlassFish modules
	 *         directory.
	 */
	public static File getCommonUtilJarInModules(final String serverHome) {
		StringBuilder commonUtilJarPath = new StringBuilder(serverHome.length() + GF_MODULES_DIR_NAME.length()
				+ GF_COMMON_UTIL_JAR.length() + 2 * OsUtils.FILE_SEPARATOR_LENGTH);
	...
	/**
	 * Build path to the supplied <code>.jar</code> file in GlassFish modules
	 * directory.
	 * <p/>
	 * 
	 * @param serverHome
	 *            GlassFish server home directory. This argument should not be
	 *            <code>null</code>.
	 * @param jarName
	 *            Supplied JAR file name. This argument should not be
	 *            <code>null</code>.
	 * @return Path to the <code>.jar</code> file in GlassFish modules directory.
	 */
	public static File getJarInModules(final String serverHome, final String jarName) {
		StringBuilder commonUtilJarPath = new StringBuilder(serverHome.length() + GF_MODULES_DIR_NAME.length()
				+ jarName.length() + 2 * OsUtils.FILE_SEPARATOR_LENGTH);
		commonUtilJarPath.append(serverHome);
		addPathElement(commonUtilJarPath, GF_MODULES_DIR_NAME);
	 ...
	 * <p/>
	 * Searches for Jersey 1.x or 2.x common (core) jersey JAR file name in
	 * GlassFish modules directory and returns <code>File</code> that was found or
	 * <code>null</code> when no such a file exists and is readable.
	 * <p/>
	 * 
	 * @param serverHome
	 *            GlassFish server home directory. This argument should not be
	...
	public static File getJerseyCommonJarInModules(final String serverHome) {
		File jerseyCommon = ServerUtils.getJarInModules(serverHome, ServerUtils.GF_JERSEY_2_COMMON_JAR);
		if (jerseyCommon.isFile() && jerseyCommon.canRead()) {
			return jerseyCommon;
		}
		jerseyCommon = ServerUtils.getJarInModules(serverHome, ServerUtils.GF_JERSEY_1_CORE_JAR);
		if (jerseyCommon.isFile() && jerseyCommon.canRead()) {
			return jerseyCommon;
		} else {
			return null;
	 ...
	 *            GlassFish server home directory. This argument should not be
	 *            <code>null</code>.
	 * @return Path to the <code>verifier.ja</code> file in GlassFish modules
	 *         directory.
	 */
	public static String getVerifierJar(final String serverHome) {
		assert serverHome != null : "GlassFish server home directory should not be null";
		boolean appendSeparator = serverHome.lastIndexOf(File.separator) + OsUtils.FILE_SEPARATOR_LENGTH != serverHome
				.length();
		StringBuilder sb = new StringBuilder(serverHome.length()
				+ (appendSeparator ? 2 * OsUtils.FILE_SEPARATOR_LENGTH : 1 * OsUtils.FILE_SEPARATOR_LENGTH)
	 ...
	 *            GlassFish server home directory. This argument should not be
	 *            <code>null</code>.
	 * @return Path to the <code>javahelp.jar</code> file in GlassFish libraries
	 *         directory.
	 */
	...
	/**
	 * Build path to the <code>glassfish-embedded-static-shell.jar</code> library in
	 ...
	 *            GlassFish server home directory. This argument should not be
	 *            <code>null</code>.
	 * @return Path to the <code>glassfish-embedded-static-shell.jar</code> file in
	 *         GlassFish embedded libraries directory.
	 */
	public static String getEmbeddedStaticShellJar(final String serverHome) {
		...
		boolean appendSeparator = serverHome.lastIndexOf(File.separator) + OsUtils.FILE_SEPARATOR_LENGTH != serverHome
				.length();
		StringBuilder sb = new StringBuilder(serverHome.length()
				+ (appendSeparator ? 3 * OsUtils.FILE_SEPARATOR_LENGTH : 2 * OsUtils.FILE_SEPARATOR_LENGTH)
				+ GF_LIB_DIR_NAME.length() + GF_EMBEDDED_DIR_NAME.length() + GF_EMBEDDED_STATIC_SHELL_JAR.length());
		sb.append(serverHome);
		if (appendSeparator) {
			sb.append(File.separator);
		}
	 ...
	 * <p/>
	 * GlassFish version is read from modules <code>common-util.jar</code> archive
	 * and <code>com.sun.appserv.server.util.Version</code> class. It's not public
	 * GlassFish API so there is no guaranty for this to work forever. However
	 * GlassFish development team promised to keep this API working the same way in
	 * GlassFish 3 and 4.
	 * <p/>
	 * 
	 * @param serverHome
	 ...
	 */
	public static Version getServerVersion(final String serverHome) {
		Version version = null;
		File commonUtilJar = getCommonUtilJarInModules(serverHome);
		if (commonUtilJar.canRead()) {
			try {
				ClassLoader cl = new URLClassLoader(new URL[] { commonUtilJar.toURI().toURL() });
				...
				try {
					...
					String versionString = getVersionString(fullVersionString);
					version = new Version(versionString);
				} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException
						| NoSuchMethodException | SecurityException ex) {
					Logger.log(Level.WARNING,
							"Cannot retrieve Glassfish version: " + commonUtilJar.getAbsolutePath() + ": ", ex);
				}
				...
				if (version == null) {
					try {
						...
						String versionString = getVersionString(manifest.getMainAttributes().getValue(BUNDLE_VERSION));
						if (versionString != null) {
							version = new Version(versionString);
						}
					} catch (IOException ioe) {
						Logger.log(Level.WARNING,
								"Cannot retrieve Glassfish version: " + commonUtilJar.getAbsolutePath() + ": ", ioe);
					}
				}
	...
	/**
	 * Build HTTP Basic authorization base64 encoded credentials argument containing
	 * user name and password.
	 * <p/>
	 * 
	 * @param user
	 *            Username to be stored into encoded argument.
	 * @param password
	 *            Password to be stored into encoded argument.
	 */
	public static String basicAuthCredentials(final String user, final String password) {
	 ...
	 * @return Value of <code>true</code> when server listener port is occupied or
	 ...
	 * @param server
	 *            GlassFish server entity.
	 * @return Value of <code>true</code> when server administrator port is occupied
	 ...
	 * 
	 * @param name
	 *            Command line argument name including dashes at the beginning.
	 * @param value
	 *            Value to be appended prefixed with single space.
	 * @return Command line argument concatenated together.
	 */
	public static String cmdLineArgument(final String name, final String value) {
	...
	/**
	 * Parse server component (application) record and add it into <code>Map</code>
	 * containing container to components <code>List</code> mapping.
	 * <p/>
	 ...
	 * 
	 * @param map
	 *            Map where new component is stored under it's container key.
	 * @param component
	 *            Component record retrieved from server.
	 * @throws <code>NullPointerException</code>
	 *             when provided map argument is <code>null</code>.
	 */
	public static void addComponentToMap(final Map<String, List<String>> map, final String component) {
		Logger.log(Level.FINER, "Processing component \"{0}\"", new Object[] { component });
		Matcher fullMatecher = ServerUtils.MANIFEST_COMPONENT_FULL_PATTERN.matcher(component);
		if (fullMatecher.matches()) {
			...
			if (moreContainers != null && moreContainers.length() > 0) {
				Matcher compMatcher = ServerUtils.MANIFEST_COMPONENT_COMP_PATTERN.matcher(moreContainers);
			...
			String containerName = container != null ? container.toString() : "null";
	...
	public static String serverLogFileRelativePath() {
		StringBuilder sb = new StringBuilder(
				GF_LOG_DIR_NAME.length() + File.separator.length() + GF_LOG_FILE_NAME.length());
	 ...
	 * @param server
	 *            GlassFish server entity
	 * @return GlassFish server domain root full path or <code>null</code> when
	 *         server domains root folder or domain name is not set.
	 */
	public static String getDomainPath(final GlassFishServer server) {
		...
		StringBuilder sb = new StringBuilder(server.getDomainsFolder().length()
				+ (appendSeparator ? OsUtils.FILE_SEPARATOR_LENGTH : 0) + domainName.length());
	...
	/**
	 * Get GlassFish server domain configuration directory full path from domain
	 * root.
	 * <p/>
	 * 
	 * @param domainDir
	 *            GlassFish server domain root full path.
	...
	public static String getDomainConfigPath(final String domainDir) {
		...
		boolean appendSeparator = domainDir.lastIndexOf(File.separator) + OsUtils.FILE_SEPARATOR_LENGTH != domainDir
				.length();
		StringBuilder sb = new StringBuilder(domainDir.length() + (appendSeparator ? OsUtils.FILE_SEPARATOR_LENGTH : 0)
	...
	/**
	 * Get GlassFish server domain configuration file (domain.xml) full path from
	 * domains root and domain name.
	 * <p/>
	 * 
	 * @param domainsRoot
	 *            GlassFish server domains root full path.
	 * @param domainName
	 *            GlassFish server domain name.
	 * @return GlassFish server domain configuration file full path
	 */
	public static String getDomainConfigFile(final String domainsRoot, final String domainName) {
		final String METHOD = "getDomainConfigFile";
		if (domainsRoot == null) {
			throw new IllegalArgumentException(LOGGER.excMsg(METHOD, "domainsRootNull"));
		}
		if (domainName == null) {
			throw new IllegalArgumentException(LOGGER.excMsg(METHOD, "domainNameNull"));
		}
		StringBuilder sb = new StringBuilder(
				domainsRoot.length() + domainName.length() + GF_DOMAIN_CONFIG_DIR_NAME.length()
						+ GF_DOMAIN_CONFIG_FILE_NAME.length() + (3 * OsUtils.FILE_SEPARATOR_LENGTH));
	 ...
	 * @param server
	 *            GlassFish server entity
	 * @return GlassFish server derby root full path or <code>null</code> when
	 *         server server installation directory is not set.
	 */
	public static String getDerbyRoot(final GlassFishServer server) {
		...
		StringBuilder sb = new StringBuilder(
				serverRoot.length() + File.separator.length() + GF_DERBY_DIR_NAME.length());
	 ...
	 * 
	 * @param javaHome
	 *            Java VM root (home) directory to be set as property value.
	 * @return GlassFish server Java VM root property to be passed to server startup
	 *         command.
	 */
	 ...
	 * @return Returns <code>true</code> if given message is server service response
	 *         while server is not yet ready or <code>false</code> otherwise.
	 */
