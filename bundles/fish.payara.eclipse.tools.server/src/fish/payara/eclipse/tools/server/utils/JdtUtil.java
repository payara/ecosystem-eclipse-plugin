...
public final class JdtUtil {
    private static String invalidJavaLocationMessage = "Java installation not found in the specified folder";

    private static String jdkIsRequiredMessage = "Java Development Kit (JDK) is required rather than a JRE";
    
    private static String javaVersionRequiredSingle = "Java {0} is required";
    
    private static String javaVersionRequiredMin = "Java {0} or higher is required";

    private static String javaVersionRequiredMax = "Java {0} or lower is required";

    private static String javaVersionRequiredOr = "Java {0} or {1} is required";

    private static String javaVersionRequiredRange = "Java {0} through {1} is required";
    ...
    public static final class JvmValidator {
        ...
        private JvmValidator(final File location) {
            ...
            this.status = (isValidJvmInstall(location) ? Status.createOkStatus()
                    : Status.createErrorStatus(invalidJavaLocationMessage));
        }
        ...
        public JvmValidator version(final VersionConstraint constraint) {
            if (this.status.ok() && constraint != null) {
                ...
                if ((min != null && !min.inclusive()) || (max != null && !max.inclusive())) {
                    throw new IllegalArgumentException();
                }
        ...
        if (location != null) {
            synchronized (jvmLocationToVersionMap) {
                ...
                if (version == null) {
                    ...
                    if (exec != null) {
                        try {
                            final Bundle bundle = Platform.getBundle("fish.payara.eclipse.tools.server.jver");
                            ...
                            final Process process = Runtime.getRuntime().exec(
                                    new String[] {
                                            ...
                                            "-cp",
                                            cp.getAbsolutePath(),
                                            "fish.payara.eclipse.tools.server.jver.JavaVersionDetector"
                                    });
