...
/******************************************************************************
 * Copyright (c) 2019-2022 Payara Foundation
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the Eclipse Public License v2.0
 * which accompanies this distribution, and is available at
 * http://www.eclipse.org/legal/epl-v20.html
...
import static fish.payara.eclipse.tools.server.PayaraServer.DEFAULT_RESTART_PATTERN;
...
public class AssembleModules {
	...
	protected IPath copyModule(IModule module, IProgressMonitor monitor) throws CoreException {
		...
		if (status != null && status.length > 0) {
			...
			for (IStatus statu : status) {
				PayaraServerPlugin.logMessage("warning copying module: " + statu.getMessage());
			}
		}
	...
	public boolean needsARedeployment() {
		...
		return childNeedsARedeployment || criticalResourceChangeThatNeedsARedeploy(
				...
				Pattern.compile(_server.getAttribute(ATTR_RESTART_PATTERN, DEFAULT_RESTART_PATTERN)));
	}

	public List<File> getModifiedSourceFiles() {
		...
		Pattern restartPattern = Pattern
				.compile(_server.getAttribute(ATTR_RESTART_PATTERN, DEFAULT_RESTART_PATTERN));
		if (deltas != null) {
			for (IModuleResourceDelta delta : deltas) {
				getModifiedSourceFiles(files, delta.getAffectedChildren(), restartPattern);
			}
		}
	...
	private List<File> getModifiedSourceFiles(final List<File> files, final IModuleResourceDelta[] deltas,
			final Pattern restartPattern) {
		if (deltas == null) {
			return files;
		}
		...
		for (IModuleResourceDelta delta : deltas) {
			...
			if (restartPattern.matcher(resource.getName()).find()) {
				File parent = resource.getModuleRelativePath().toFile();
				files.add(new File(parent, resource.getName()));
				continue;
			}
