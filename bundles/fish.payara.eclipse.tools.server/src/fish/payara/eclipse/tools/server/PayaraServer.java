...
import static fish.payara.eclipse.tools.server.Messages.facetNotSupported;
import static fish.payara.eclipse.tools.server.Messages.invalidPortNumbers;
import static fish.payara.eclipse.tools.server.Messages.pathDoesNotExist;
import static fish.payara.eclipse.tools.server.Messages.pathNotDirectory;
import static fish.payara.eclipse.tools.server.Messages.pathNotValidDomain;
import static fish.payara.eclipse.tools.server.Messages.pathNotWritable;
import static fish.payara.eclipse.tools.server.Messages.serverWithSameDomainPathExisting;
...
import static fish.payara.eclipse.tools.server.utils.ModuleUtil.isWebModule;
import static fish.payara.eclipse.tools.server.utils.PayaraLocationUtils.find;
import static fish.payara.eclipse.tools.server.utils.Utils.canWrite;
import static fish.payara.eclipse.tools.server.utils.Utils.getAppWebContextRoot;
...
import fish.payara.eclipse.tools.server.sdk.data.GlassFishAdminInterface;
import fish.payara.eclipse.tools.server.sdk.server.parser.HttpData;
import fish.payara.eclipse.tools.server.sdk.server.parser.HttpListenerReader;
import fish.payara.eclipse.tools.server.sdk.server.parser.NetworkListenerReader;
import fish.payara.eclipse.tools.server.sdk.server.parser.TargetConfigNameReader;
...
public final class PayaraServer extends ServerDelegate implements IURLProvider {
	...
	public static final String ATTR_HOTDEPLOY = "glassfish.hotDeploy"; //$NON-NLS-1$
	public static final String ATTR_USEANONYMOUSCONNECTIONS = "glassfish.useAnonymousConnection"; //$NON-NLS-1$
	public static final String ATTR_RESTART_PATTERN = "glassfish.restartPattern"; //$NON-NLS-1$
	public static final String ATTR_ATTACH_DEBUGGER_EARLY = "glassfish.attachDebuggerEarly"; //$NON-NLS-1$
	public static final String DEFAULT_RESTART_PATTERN = "\\.(jar|class|xml)$"; //$NON-NLS-1$
	...
	public static String createServerNameWithDomain(String serverName, String domainPath) {
		int domainStartPos = serverName.lastIndexOf("[");
		if (domainStartPos == -1) {
			return serverName + " [" + lastSegment(domainPath) + "]";
		}

		return serverName.substring(0, domainStartPos) + "[" + lastSegment(domainPath) + "]";
	}
	...
	public IStatus validateDomainLocation() {
		return validateDomainLocation(getDomainPath());
	}

	public IStatus validateDomainLocation(String domainPath) {
		if (isRemote()) {
			return OK_STATUS;
		}
...
//            getModel().setServerPort(getPort());
	...
	public void firePropertyChangeEvent(String propertyName, Object oldValue, Object newValue) {
		...
		try {
			for (PropertyChangeListener listener : propChangeListeners) {
				try {
	...
	public int getDebugPort() {
		return getAttribute(ATTR_DEBUG_PORT, 9009);
	}

	public void setDebugPort(int value) {
		setAttribute(ATTR_DEBUG_PORT, value);
	}
	...
	public int getAdminPort() {
		return getAttribute(ATTR_ADMINPORT, 4848);
	}
	...
	public void setDomainPath(String value) {
		setAttribute(ATTR_DOMAINPATH, value);
	}
	...
	public void setInstanceType(String type) {
		if (type == null) {
			setAttribute(PROP_INSTANCE_TYPE, (String) null);
		} else {
			setAttribute(PROP_INSTANCE_TYPE, type);
		}
	}

	public String getHostPath() {
		return this.getAttribute(PROP_HOST_PATH, (String) null);
	}

	public void setHostPath(String path) {
		if (path == null) {
			setAttribute(PROP_HOST_PATH, (String) null);
		} else {
			setAttribute(PROP_HOST_PATH, path);
		}
	}

	public String getContainerPath() {
		return this.getAttribute(PROP_CONTAINER_PATH, (String) null);
	}

	public void setContainerPath(String path) {
		if (path == null) {
			setAttribute(PROP_CONTAINER_PATH, (String) null);
		} else {
			setAttribute(PROP_CONTAINER_PATH, path);
		}
	}
	...
	public String getRestartPattern() {
		return getAttribute(ATTR_RESTART_PATTERN, DEFAULT_RESTART_PATTERN);
	}

	public void setRestartPattern(String value) {
		setAttribute(ATTR_RESTART_PATTERN, value);
	}
	...
	public void setKeepSessions(boolean value) {
		setAttribute(ATTR_KEEPSESSIONS, value);
	}

	public boolean getHotDeploy() {
		return getAttribute(ATTR_HOTDEPLOY, true);
	}

	public void setHotDeploy(boolean value) {
		setAttribute(ATTR_HOTDEPLOY, value);
	}

	public boolean getAttachDebuggerEarly() {
		return getAttribute(ATTR_ATTACH_DEBUGGER_EARLY, false);
	}

	public void setAttachDebuggerEarly(boolean value) {
		setAttribute(ATTR_ATTACH_DEBUGGER_EARLY, value);
	}
	...
	public void setJarDeploy(boolean value) {
		setAttribute(ATTR_JARDEPLOY, value);
	}
