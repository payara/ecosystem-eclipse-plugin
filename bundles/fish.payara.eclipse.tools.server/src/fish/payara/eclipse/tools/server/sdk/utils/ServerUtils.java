...
package fish.payara.eclipse.tools.server.sdk.utils;
...
import fish.payara.eclipse.tools.server.sdk.admin.CommandException;
import fish.payara.eclipse.tools.server.sdk.data.GlassFishContainer;
...
public class ServerUtils {
    ...
    /**
     * Filtering the set of directories with GlassFish server home subdirectory shown to the user in
     * file filter.
     */
    ...
    /**
     * GlassFish Java VM system environment <code>AS_JAVA</code> variable name. This value should be
     * equal to system environment <code>JAVA_HOME</code> value.
     */
     ...
     * <p/>
     * It's value says it is server installation root but in reality it is just <code>glassfish</code>
     * subdirectory under server installation root which we usually call server home.
     */
    ...
    /**
     * Manifest attribute Bundle-Version containing GlassFish version <code>String</code>.
     */
    ...
    /**
     * Regex pattern to retrieve version string like 3.1.2.2 from full version string.
     */
    ...
    /**
     * GlassFish server service response while server is not yet ready. Copy-pasted from
    ...
    /**
     * REGEX expression used to split resources returned in <code>Manifest</code> object from HTTP
     * response.
     */
    public static final String MANIFEST_RESOURCES_SEPARATOR = "[,;]";

    /**
     * REGEX expression used to split components (applications) returned in <code>Manifest</code> object
     * from HTTP response.
     */
    public static final String MANIFEST_COMPONENTS_SEPARATOR = ";";

    /**
     * REGEX expression used to extract component name and containers from <code>Manifest</code>
     * attribute.
     */
    private static final String MANIFEST_COMPONENT_FULL_REGEX = " *([^ ]+) +< *([^ ,]+) *((?:, *[^ ,]+ *)*)> *";

    /**
     * REGEX expression used to extract additional containers from containers list of
     * <code>Manifest</code> attribute.
     */
    private static final String MANIFEST_COMPONENT_COMP_REGEX = ", *([^ ,]+) *";

    /**
     * REGEX pattern used to extract component name and containers from <code>Manifest</code> attribute.
     */
    private static final Pattern MANIFEST_COMPONENT_FULL_PATTERN = Pattern.compile(MANIFEST_COMPONENT_FULL_REGEX,
            Pattern.CASE_INSENSITIVE);

    /**
     * REGEX pattern used to extract additional containers from containers list of <code>Manifest</code>
     * attribute.
     */
    ...
    /**
     * Build path to the <code>jersey-common.jar</code> or <code>jersey-core.jar</code> file in
     * GlassFish modules directory.
     * <p/>
     * Searches for Jersey 1.x or 2.x common (core) jersey JAR file name in GlassFish modules directory
     ...
     * <p/>
     * GlassFish version is read from modules <code>common-util.jar</code> archive and
     * <code>com.sun.appserv.server.util.Version</code> class. It's not public GlassFish API so there is
    ...
    /**
     * Parse server component (application) record and add it into <code>Map</code> containing container
     * to components <code>List</code> mapping.
     * <p/>
