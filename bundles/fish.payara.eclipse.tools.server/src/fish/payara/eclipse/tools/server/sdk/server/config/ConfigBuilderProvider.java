 ...
 * there must be one configuration class instance for every single GlassFish server version. Also
 * every single server instance has it's own directory structure which is used to search for
 * modules. Because of that every single GlassFish server instance must have it's own configuration
...
public class ConfigBuilderProvider {
    ...
    /**
     * Remove configuration builder instance for given GlassFish server entity instance.
     * <p/>
